#!/usr/bin/env python3
"""
Create a Word .docx file for Chapter 17
"Robotics and Autonomous Systems in Healthcare".

Uses the raw-OOXML approach (python-docx is not available in this sandbox) and
embeds the generated figures inline where the "[Insert Figure 17.N here]"
placeholders appear. Renders markdown pipe-tables as native Word tables and
formats references with a hanging indent.

Usage:
    python3 create_healthcare_docx.py
"""
...
MD_FILE = '/projects/sandbox/AMMAN/Chapter_17_Robotics_Autonomous_Systems_Healthcare.md'
DOCX_FILE = '/projects/sandbox/AMMAN/Chapter_17_Robotics_Autonomous_Systems_Healthcare.docx'
FIG_DIR = '/projects/sandbox/AMMAN/healthcare_figures'

# Map figure key (e.g. "17.1") -> (image filename, caption)
FIGURE_FILES = {
    '17.1': ('Figure_17_1_Closed_Loop_Drug_Delivery.png',
             'Figure 17.1. Architecture of a closed-loop drug-delivery system: an optimizing controller wrapped by a conservative safety supervisor.'),
    '17.2': ('Figure_17_2_Surgical_Control_Architecture.png',
             'Figure 17.2. Layered control architecture of an AI-assisted surgical system, in which learning-based components operate within safety constraints.'),
    '17.3': ('Figure_17_3_Human_Robot_Collaboration.png',
             'Figure 17.3. Framework for human-robot collaboration in healthcare: shared situational awareness, intent communication, and control arbitration.'),
    '17.4': ('Figure_17_4_Shared_Autonomy_Spectrum.png',
             'Figure 17.4. The shared-autonomy spectrum, relating machine authority to the transformed human role and the rising safety-assurance burden.'),
}
...
def make_table(headers, rows):
    ...
    for h in headers:
        tbl += '<w:tc>'
        tbl += f'<w:tcPr><w:tcW w:w="{col_w}" w:type="dxa"/><w:shd w:val="clear" w:color="auto" w:fill="D9E2F3"/></w:tcPr>'
        tbl += ('<w:p><w:pPr><w:pStyle w:val="Normal"/><w:jc w:val="center"/>'
                '<w:spacing w:after="40" w:line="240" w:lineRule="auto"/></w:pPr>'
                + make_run(h.strip(), bold=True) + '</w:p>')
        tbl += '</w:tc>'
    tbl += '</w:tr>'
    ...
    for row in rows:
        ...
        for cell in row:
            ...
            tbl += ('<w:p><w:pPr><w:pStyle w:val="Normal"/><w:jc w:val="left"/>'
                    '<w:spacing w:after="40" w:line="240" w:lineRule="auto"/></w:pPr>'
                    + parse_inline(cell.strip()) + '</w:p>')
            tbl += '</w:tc>'
    ...
    tbl += '</w:tbl>'
    # a trailing empty paragraph so tables don't collide with following content
...
def md_to_body(md_text, image_rels):
    ...
    while i < len(lines):
        ...
        # Table caption line: **Table 17.N.** ...
        if line.strip().startswith('**Table 17.'):
        ...
        m = re.match(r'\[Insert Figure (17\.\d+) here\]', line.strip())
        if m:
            fig_key = m.group(1)
            entry = FIGURE_FILES.get(fig_key)
            fpath = os.path.join(FIG_DIR, entry[0]) if entry else None
            if fpath and os.path.exists(fpath):
                img_counter += 1
                rel_id = f'rIdImg{img_counter}'
                image_rels.append((rel_id, entry[0]))
                px_w, px_h = png_dimensions(fpath)
                cx = int(TARGET_WIDTH_INCHES * EMU_PER_INCH)
                cy = int(cx * px_h / px_w)
                elements.append(
                    make_image_paragraph(rel_id, img_counter, cx, cy, entry[0]))
                elements.append(make_paragraph(entry[1], 'FigureCaption'))
            else:
