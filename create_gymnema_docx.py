...
Potential of Gymnema sylvestre: From Traditional Knowledge to Precision
Therapeutics".

Uses the raw-OOXML approach established in create_agentic_docx.py (python-docx
is not available in this sandbox) and EMBEDS the generated figures inline where
the "[Insert Figure N here]" placeholders appear. APA-style reference entries
(which begin with author surnames rather than "[n]") are rendered with a
hanging indent.

Usage:
    python3 create_gymnema_docx.py
"""
...
MD_FILE = '/projects/sandbox/AMMAN/Chapter_Gymnema_AI_Antidiabetic.md'
DOCX_FILE = '/projects/sandbox/AMMAN/Chapter_Gymnema_AI_Antidiabetic.docx'
FIG_DIR = '/projects/sandbox/AMMAN/gymnema_figures'

# Map figure number -> image filename
FIGURE_FILES = {
    1: 'Figure_1_Profiling_Workflow.png',
    2: 'Figure_2_Antidiabetic_Mechanisms.png',
    3: 'Figure_3_Integrative_Pipeline.png',
    4: 'Figure_4_Model_Performance.png',
}
...
def make_table(headers, rows):
    ...
    for h in headers:
        ...
        tbl += f'<w:p><w:pPr><w:jc w:val="center"/></w:pPr>{parse_inline(h.strip())}</w:p>'
        tbl += '</w:tc>'
    tbl += '</w:tr>'
    ...
    for row in rows:
        ...
        for cell in row:
            tbl += '<w:tc>'
            tbl += f'<w:p><w:pPr><w:spacing w:after="40"/></w:pPr>{parse_inline(cell.strip())}</w:p>'
            tbl += '</w:tc>'
...
def md_to_body(md_text, image_rels):
    ...
    while i < len(lines):
        ...
        # Figure caption line
        if line.strip().startswith('Figure ') and len(line.strip()) > 7 and line.strip()[7:8].isdigit():
            elements.append(make_paragraph(line.strip(), 'FigureCaption'))
            i += 1
            continue

        # Table caption line (not a table row)
        if line.strip().startswith('Table ') and len(line.strip()) > 6 and line.strip()[6:7].isdigit() and '|' not in line:
            elements.append(make_paragraph(line.strip(), 'TableCaption'))
            i += 1
            continue

        # References: APA entries begin with author surnames, not "[n]".
        if in_references and not line.strip().startswith('**Note:**'):
