#!/usr/bin/env python3
"""
Create a Word .docx file for Chapter 5
...
embeds the generated figures inline where "[Insert Figure 5.N here]" placeholders
appear. Renders markdown tables, numbered equations ("[EQ] ... (5.x)"), and a
hanging-indent reference list. Body text is Times New Roman 12 pt with 1.5 line
spacing, per the publisher's formatting guidelines.

Usage:
    python3 create_hitl_docx.py
"""
...
MD_FILE = '/projects/sandbox/AMMAN/Chapter_5_HITL_XAI_Mission_Control.md'
DOCX_FILE = '/projects/sandbox/AMMAN/Chapter_5_HITL_XAI_Mission_Control.docx'
FIG_DIR = '/projects/sandbox/AMMAN/hitl_figures'

# Map figure number (the N in "Figure 5.N") -> image filename
FIGURE_FILES = {
    1: 'Figure_5_1_Latency_Autonomy.png',
    2: 'Figure_5_2_XAI_Taxonomy.png',
    3: 'Figure_5_3_Reference_Architecture.png',
    4: 'Figure_5_4_Tradeoff_Calibration.png',
}
...
# Body font: Times New Roman 12 pt (sz 24 half-points); line spacing 1.5 (line=360).
...
def md_to_body(md_text, image_rels):
    ...
    while i < len(lines):
        ...
        # Numbered equation:  [EQ] ... (5.x)
        if line.strip().startswith('[EQ]'):
            eq_text = line.strip()[4:].strip()
            elements.append(make_paragraph(eq_text, 'Equation'))
            i += 1
            continue
        ...
        # Figure placeholder -> embed the actual image ("[Insert Figure 5.N here]")
        m = re.match(r'\[Insert Figure 5\.(\d+) here\]', line.strip())
        if m:
        ...
        # Table placeholder -> skip (caption + table follow immediately)
        if line.strip().startswith('[Insert Table'):
            i += 1
            continue

        # Figure caption ("Figure 5.N. ...")
        if line.strip().startswith('Figure 5.'):
            elements.append(make_paragraph(line.strip(), 'FigureCaption'))
            i += 1
            continue

        # Table caption ("Table 5.N. ...")
        if line.strip().startswith('Table 5.') and '|' not in line:
