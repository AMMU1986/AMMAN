...
"[Insert Figure N here]" placeholders appear. Reference entries are formatted in
APA style (author-year, hanging indent), and figure/table caption paragraphs are
styled distinctly.

Usage:
    python3 create_intersectionality_docx.py
"""
...
MD_FILE = '/projects/sandbox/AMMAN/Chapter_Intersectionality_Climate_Resilience.md'
DOCX_FILE = '/projects/sandbox/AMMAN/Chapter_Intersectionality_Climate_Resilience.docx'
FIG_DIR = '/projects/sandbox/AMMAN/intersectionality_figures'

# Map figure number -> image filename
FIGURE_FILES = {
    1: 'Figure_1_Intersectional_Framework.png',
    2: 'Figure_2_Vulnerability_Pathways.png',
    3: 'Figure_3_Resilience_Ecosystem.png',
    4: 'Figure_4_Action_Framework.png',
}
...
def md_to_body(md_text, image_rels):
    ...
    while i < len(lines):
        ...
        # Figure caption paragraph (starts with **Figure N.**)
        if re.match(r'^\*\*Figure \d+\.\*\*', line.strip()):
            elements.append(make_paragraph(line.strip(), 'FigureCaption'))
            i += 1
            continue

        # Table caption paragraph (starts with **Table N.**), not a table row
        if re.match(r'^\*\*Table \d+\.\*\*', line.strip()):
            elements.append(make_paragraph(line.strip(), 'TableCaption'))
            i += 1
            continue

        # Reference entries (APA: any non-empty line within the References section)
        if in_references:
