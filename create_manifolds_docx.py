...
not available in this sandbox). It:
  * EMBEDS the four generated figures inline at the "[Insert Figure N here]"
    placeholders,
  * renders the markdown pipe-tables as bordered Word tables,
  * turns the corresponding-author email into a clickable mailto hyperlink.

Usage:
    python3 create_manifolds_docx.py
"""
...
MD_FILE = '/projects/sandbox/AMMAN/Chapter_Manifolds_GR_Cosmology.md'
DOCX_FILE = '/projects/sandbox/AMMAN/Chapter_Manifolds_GR_Cosmology.docx'
FIG_DIR = '/projects/sandbox/AMMAN/manifold_figures'

FIGURE_FILES = {
    1: 'Figure_1_Manifold_Charts.png',
    2: 'Figure_2_Curvature_Einstein.png',
    3: 'Figure_3_FLRW_Geometries.png',
    4: 'Figure_4_Advanced_Tools.png',
}
...
def make_hyperlink(text, rel_id):
    """A clickable hyperlink run referencing an external relationship."""
    run = (f'<w:r><w:rPr><w:rStyle w:val="Hyperlink"/></w:rPr>'
           f'<w:t xml:space="preserve">{escape_xml(text)}</w:t></w:r>')
    return f'<w:hyperlink r:id="{rel_id}">{run}</w:hyperlink>'
...
def make_email_paragraph(text, style, hyperlink_rels):
    """Render a line containing an email address with a mailto hyperlink."""
    m = EMAIL_RE.search(text)
    if not m:
        return make_paragraph(text, style)
    email = m.group(0)
    before, after = text[:m.start()], text[m.end():]
    rel_id = f'rIdLink{len(hyperlink_rels) + 1}'
    hyperlink_rels.append((rel_id, f'mailto:{email}'))
    ppr = f'<w:pPr><w:pStyle w:val="{style}"/></w:pPr>'
    runs = ''
    if before:
        runs += make_run(before)
    runs += make_hyperlink(email, rel_id)
    if after:
        runs += make_run(after)
...
def md_to_body(md_text, image_rels, hyperlink_rels):
    elements = []
    lines = md_text.split('\n')
    i = 0
    in_references = False
    ...
    while i < len(lines):
        ...
        # Author / affiliation / email block right after the title
        stripped = line.strip()
        if title_done and not in_references and i < 12 and EMAIL_RE.search(stripped):
            elements.append(make_email_paragraph(stripped, 'Author', hyperlink_rels))
            i += 1
            continue
        if (title_done and not in_references and
                (stripped.startswith('Amman Jakhar') or
                 stripped.startswith('1 Department') or
                 stripped.startswith('**Keywords'))):
            elements.append(make_paragraph(stripped, 'Author'
                                           if not stripped.startswith('**Keywords') else 'Normal'))
            i += 1
            continue
...
def build_word_rels(image_rels, hyperlink_rels):
    rels = ['<?xml version="1.0" encoding="UTF-8" standalone="yes"?>',
            '<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">',
    ...
    for rel_id, target in hyperlink_rels:
        rels.append(
            f'  <Relationship Id="{rel_id}" '
            'Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" '
            f'Target="{escape_xml(target)}" TargetMode="External"/>')
...
def create_docx(md_filepath, output_filepath):
    ...
    image_rels = []
    hyperlink_rels = []
    body_xml = md_to_body(md_content, image_rels, hyperlink_rels)
...
{body_xml}
    ...
    word_rels = build_word_rels(image_rels, hyperlink_rels)
    ...
    print(f"Embedded {len(image_rels)} figure(s): {', '.join(fn for _, fn in image_rels)}")
    print(f"Hyperlink(s): {', '.join(t for _, t in hyperlink_rels)}")
