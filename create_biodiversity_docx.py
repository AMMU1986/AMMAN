...
Adapts the raw-OOXML approach from create_agentic_docx.py (python-docx is not
available in this sandbox) and additionally:
  * embeds the four generated figures inline at the "[Insert Figure N here]"
    placeholders, and
  * converts Markdown hyperlinks of the form [text](url) into clickable Word
    hyperlinks (external relationships with TargetMode="External").

Usage:
    python3 create_biodiversity_docx.py
"""
...
MD_FILE = '/projects/sandbox/AMMAN/Chapter_Biodiversity_AI_Prediction.md'
DOCX_FILE = '/projects/sandbox/AMMAN/Chapter_Biodiversity_AI_Prediction.docx'
FIG_DIR = '/projects/sandbox/AMMAN/biodiversity_figures'

# Map figure number -> image filename
FIGURE_FILES = {
    1: 'Figure_1_Cognitive_Digital_Twin.png',
    2: 'Figure_2_Data_Fusion_Pipeline.png',
    3: 'Figure_3_Methods_GNN_Workflow.png',
    4: 'Figure_4_Stressor_Cascade_Decision.png',
}

EMU_PER_INCH = 914400
TARGET_WIDTH_INCHES = 6.0  # fit within 1-inch margins on Letter page

# Collectors populated during body construction
IMAGE_RELS = []       # list of (rel_id, filename)
HYPERLINK_RELS = []    # list of (rel_id, url)
...
def make_hyperlink(text, url):
    """Register an external relationship and return a w:hyperlink element."""
    ...
    return (
        f'<w:hyperlink r:id="{rel_id}" w:history="1">{run}</w:hyperlink>'
    )


def parse_inline(text):
    """Convert inline markdown (links, bold, italic) to OOXML runs."""
    runs = []
    # First split out hyperlinks [text](url)
    link_pattern = r'\[([^\]]+)\]\((https?://[^)]+)\)'
    pos = 0
    for m in re.finditer(link_pattern, text):
        if m.start() > pos:
            runs.append(_parse_emphasis(text[pos:m.start()]))
        runs.append(make_hyperlink(m.group(1), m.group(2)))
        pos = m.end()
    if pos < len(text):
        runs.append(_parse_emphasis(text[pos:]))
    return ''.join(runs)
...
def md_to_body(md_text):
    ...
    while i < len(lines):
        ...
        # Abstract paragraph styled as block quote
...
def build_word_rels():
    ...
    for rel_id, url in HYPERLINK_RELS:
        rels.append(
            ...
            f'Target="{escape_xml(url)}" TargetMode="External"/>')
...
{body_xml}
    ...
    print(f"Inserted {len(HYPERLINK_RELS)} hyperlink(s): "
          f"{', '.join(u for _, u in HYPERLINK_RELS)}")
