...
Uses the same raw-OOXML approach as create_agentic_docx.py (python-docx is not
available in this sandbox) and EMBEDS the generated figures inline where the
"[Insert Figure N here]" placeholders appear.

Difference from create_agentic_docx.py: this chapter uses APA author-date
references (not numbered [n] entries), so the References section applies the
"References" (hanging-indent) style to every entry line rather than matching a
leading bracketed number.

Usage:
    python3 create_research_docx.py
"""
...
MD_FILE = '/projects/sandbox/AMMAN/Chapter_AI_Research_Support_RDM_Workflows.md'
DOCX_FILE = '/projects/sandbox/AMMAN/AI_Research_Support_RDM_Workflows_Chapter.docx'
FIG_DIR = '/projects/sandbox/AMMAN/research_figures'

# Map figure number -> image filename
FIGURE_FILES = {
    1: 'Figure_1_Research_Life_Cycle.png',
    2: 'Figure_2_RDM_Pipeline.png',
    3: 'Figure_3_Research_Workflow.png',
    4: 'Figure_4_Responsible_AI_Framework.png',
}
...
def md_to_body(md_text, image_rels):
    ...
    while i < len(lines):
        ...
        # Table placeholder / caption line
        ...
        # APA reference entries: any content line inside the References section
        if in_references:
