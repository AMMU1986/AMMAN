#!/usr/bin/env python3
"""
Create a Word .docx for Chapter 6 (AI in Pharmacokinetics and Pharmacodynamics).
...
MD_FILE = os.path.join(BASE, 'Chapter_6_AI_Pharmacokinetics_Pharmacodynamics.md')
OUT_FILE = os.path.join(BASE, 'Chapter_6_AI_Pharmacokinetics_Pharmacodynamics.docx')
FIG_DIR = os.path.join(BASE, 'ch6_figures')

# Map each figure caption prefix to its PNG file
FIGURE_MAP = {
    'Figure 6.1.': 'Figure_6_1_ADME_Workflow.png',
    'Figure 6.2.': 'Figure_6_2_Exposure_Response.png',
    'Figure 6.3.': 'Figure_6_3_Hybrid_PBPK.png',
    'Figure 6.4.': 'Figure_6_4_Precision_Dosing.png',
}
...
def make_image_paragraph(rel_id, cx_emu, cy_emu, name):
    """Inline image paragraph (centred)."""
...
def build():
    ...
    rel_counter = 10     # image rIds start high to avoid collision
    in_references = False
    in_abstract = False
    i = 0
    ...
    while i < len(lines):
        ...
        if fig_key:
            ...
            # scale to ~6 inch width max (5486400 EMU = 6in)
            max_w = 5486400
            cx = w * EMU_PER_PX
            cy = h * EMU_PER_PX
            if cx > max_w:
            ...
            continue
        # Table caption
        if re.match(r'^Table 6\.\d+\.', line.strip()):
        ...
        if in_references and re.match(r'^\[\d+\]', line.strip()):
            ...
            i += 1
            continue
        # Abstract body paragraphs use the italic indented Abstract style
        if in_abstract:
