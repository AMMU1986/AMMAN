#!/usr/bin/env python3
"""
Create a Word .docx file for Chapter 1: AI-Driven Smart Computing.
Uses raw OOXML (ZIP + XML) since python-docx is not available in this sandbox.
Includes embedded PNG figures.
"""
...
MD_FILE = os.path.join(SCRIPT_DIR, 'Chapter_AI_Smart_Computing.md')
OUTPUT_FILE = os.path.join(SCRIPT_DIR, 'Chapter_AI_Smart_Computing.docx')
FIGURES_DIR = os.path.join(SCRIPT_DIR, 'smart_computing_figures')

# Figure mapping: placeholder text -> (filename, rId)
FIGURE_MAP = {
    'Figure 1': ('Figure_1_Architecture.png', 'rId10'),
    'Figure 2': ('Figure_2_Convergence.png', 'rId11'),
    'Figure 3': ('Figure_3_Applications.png', 'rId12'),
    'Figure 4': ('Figure_4_Sustainability.png', 'rId13'),
}
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  <Relationship Id="rId10" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/Figure_1_Architecture.png"/>
  <Relationship Id="rId11" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/Figure_2_Convergence.png"/>
  <Relationship Id="rId12" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/Figure_3_Applications.png"/>
  <Relationship Id="rId13" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/Figure_4_Sustainability.png"/>
</Relationships>'''
...
def make_image_paragraph(rid, width_emu=5400000, height_emu=3700000):
    """Create paragraph with inline image using DrawingML."""
    return f'''<w:p>
...
def make_table(headers, rows):
    ...
    tbl += '<w:tblW w:w="9200" w:type="dxa"/>'
...
def get_figure_for_insert(line):
    """Check if this line is a figure insert placeholder and return the rId."""
    for fig_key, (fname, rid) in FIGURE_MAP.items():
        if f'[Insert {fig_key}' in line:
            return rid
    return None
...
def md_to_body(md_text):
    ...
    while i < len(lines):
        ...
        # Figure insert placeholder - embed actual image
        rid = get_figure_for_insert(line.strip())
        if rid:
            fig_insert_count += 1
            elements.append(make_image_paragraph(rid))
            i += 1
            continue
...
def create_docx():
    """Create the .docx file from markdown source with embedded images."""
    ...
    with zipfile.ZipFile(OUTPUT_FILE, 'w', zipfile.ZIP_DEFLATED) as zf:
        ...
        for fig_key, (fname, rid) in FIGURE_MAP.items():
            ...
            if os.path.exists(fig_path):
                zf.write(fig_path, f'word/media/{fname}')
