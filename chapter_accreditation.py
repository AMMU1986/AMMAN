...
References are numbered [1]-[52] in strict serial order of first appearance.
"""
...
def create_styles():
    ...
    SubElement(h1_rPr, make_tag('w', 'b'))
    ...
    SubElement(h2_rPr, make_tag('w', 'b'))
...
def create_document(paragraphs):
    ...
    pgSz.set(make_tag('w', 'w'), '12240')
    pgSz.set(make_tag('w', 'h'), '15840')
...
def get_chapter_content():
    ...
    All citations are numbered [1]-[52] in strict serial order of first appearance.
    """
    content = []
    ...
    content.append(('', 'blank'))
    content.append(('The landscape of higher education is undergoing a period of profound transformation. Traditional boundaries\u2014between disciplines, between institutions, between nations, and between the academy and the world of work\u2014are dissolving at an accelerating pace. In this environment of radical change, the question of how institutions demonstrate their quality, relevance, and fitness for purpose has become one of the most consequential debates in educational policy [1]. At the center of this debate stands accreditation: a process that is simultaneously ancient in its collegial principles and urgently modern in its demands.', 'body_indent'))
    content.append(('', 'blank'))
    content.append(('Accreditation in higher education has long functioned as the primary mechanism through which institutions voluntarily submit to external review to demonstrate that they meet established standards of quality [2]. Yet the perception and purpose of this process have shifted dramatically over the past three decades. What was once a largely private conversation among academic peers has become a public instrument of accountability, scrutinized by legislators, journalists, and an increasingly skeptical public demanding evidence that the substantial investment in higher education yields meaningful returns [3, 4].', 'body_indent'))
    content.append(('', 'blank'))
    ...
    content.append(('', 'blank'))
    content.append(('The governmental dimension of this stakeholder mandate is perhaps the most consequential. In the United States, institutional accreditation serves as the gateway to federal financial aid\u2014a mechanism that channels over $150 billion annually to students and, through them, to institutions [15]. This linkage between accreditation status and access to public funding has transformed what was once a voluntary professional process into an effective governmental requirement, dramatically raising the stakes of accreditation decisions and intensifying demands for transparency and public accountability.', 'body_indent'))
    content.append(('', 'blank'))
    ...
    content.append(('', 'blank'))
    content.append(('Mission serves as the compass in this strategic alignment. The self-study process\u2014if genuinely conducted\u2014forces institutions to critically evaluate whether their current strategic direction remains aligned with their foundational purposes and with evolving market realities [23]. In a rapidly changing environment, yesterday\u2019s strategic plan may no longer be responsive to today\u2019s challenges. An institution whose mission emphasizes preparation for the workforce must continuously evaluate whether its programs align with emerging employment needs. A liberal arts college committed to developing critical thinking must assess whether its pedagogical approaches remain effective for current student populations. The self-study provides a structured occasion for this essential strategic reflection.', 'body_indent'))
    content.append(('', 'blank'))
    content.append(('Resource allocation represents a critical bridge between strategic insight and institutional action. The findings from a thorough self-study\u2014revealing which programs are thriving, which are struggling, which support services are effective, and which investments are yielding diminishing returns\u2014provide an evidence base for difficult resource decisions that might otherwise be made on the basis of political power, historical precedent, or institutional inertia [41]. When accreditation findings demonstrate that a program is producing excellent outcomes with minimal resources, or conversely that substantial investment in another area is yielding disappointing results, institutional leaders gain both the information and the political legitimacy to advocate for strategic reallocation.', 'body_indent'))
    content.append(('', 'blank'))
    content.append(('The strategic alignment of accreditation with institutional planning also creates opportunities for building institutional capacity. When an institution identifies through its self-study that data systems are inadequate, that assessment expertise is lacking, or that communication structures impede collaborative work, the accreditation process provides both the rationale and the urgency for addressing these capacity gaps [42]. Investments in institutional research infrastructure, professional development for assessment coordinators, and technology platforms for data management can be justified not merely as accreditation expenses but as strategic investments in the institution\u2019s long-term capacity for evidence-based decision-making. The accreditation cycle thus becomes a mechanism for systematically building the organizational infrastructure that supports sustained quality improvement.', 'body_indent'))
    content.append(('', 'blank'))
    content.append(('', 'blank'))
    ...
    # References - numbered [1]-[52] in strict serial order of first appearance
...
if __name__ == '__main__':
    ...
    # Verify all 52 references present
    refs = [t for t, tp in content if tp == 'reference']
    print(f"Number of references: {len(refs)}")
    
    # Verify serial order
    for i, ref in enumerate(refs, 1):
        expected = f'[{i}]'
        assert ref.startswith(expected), f"Reference {i} does not start with {expected}: {ref[:30]}"
    print("All references are in strict serial order [1]-[52].")
